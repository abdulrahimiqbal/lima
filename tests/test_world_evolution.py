...
def test_pressure_height_route_integration_wave_compiles_third_tranche(
    tmp_path: Path,
) -> None:
    app = create_app(_settings(tmp_path))
    client = TestClient(app)
    ...
    response = client.post(
        f"/api/campaigns/{campaign_id}/world-evolution/pressure-height-route-integration-wave",
        json={
            "max_probes": 13,
            "submit_after_compile": False,
        },
    ...
    payload = response.json()
    assert payload["compiled_probe_count"] == 13
    assert "integrated exactness+drift certificate is definable" in payload["integration_gates"]
    assert "R23 bridge assumptions imply the R22 pressure-height invariant" in payload["integration_gates"]
    assert "density-zero and ordinary Collatz pullback remain explicitly named debts" in payload["remaining_debt_gates"]
    assert "integration target remains free of reachability and termination fields" in payload["remaining_debt_gates"]
    assert "Tranche 3 of the SCC drift/exactness gauntlet" in payload["tranche_summary"]
    assert payload["decisive_probe_ids"]
    assert any("proof spine" in item for item in payload["expected_learning"])


def test_pressure_height_final_closure_wave_compiles_final_phase(
    tmp_path: Path,
) -> None:
    app = create_app(_settings(tmp_path))
    client = TestClient(app)
    ...
    response = client.post(
        f"/api/campaigns/{campaign_id}/world-evolution/pressure-height-final-closure-wave",
        json={
            "max_probes": 13,
            "submit_after_compile": False,
        },
    ...
    payload = response.json()
    assert payload["compiled_probe_count"] == 13
    assert "no-dangerous-frontier global closure target is definable" in payload["closure_gates"]
    assert "density-zero Composite Scarcity closure is stated without reachability" in payload["closure_gates"]
    assert "ordinary Collatz pullback target is definable from pressure-height closure" in payload["pullback_gates"]
    assert "ordinary termination follows from closure plus base coverage" in payload["pullback_gates"]
    assert "density theorem that assumes termination is rejected" in payload["anti_circularity_gates"]
    assert "final theorem target has no hidden reachability or termination field" in payload["anti_circularity_gates"]
    assert "Final phase wave" in payload["phase_summary"]
    assert payload["decisive_probe_ids"]
    assert any("final gap" in item for item in payload["expected_learning"])
