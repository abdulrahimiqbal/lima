...
class GlobalForcingHuntWaveRequest(BaseModel):
    world_id: str | None = None
    max_probes: int = Field(default=12, ge=8, le=15)
    submit_after_compile: bool = False


class GlobalForcingHuntWaveRun(BaseModel):
    id: str = Field(default_factory=lambda: f"GF-{uuid4().hex[:10]}")
    campaign_id: str
    world_id: str | None = None
    world_label: str | None = None
    compiled_probe_count: int = 0
    submitted_probe_count: int = 0
    probe_ids: list[str] = Field(default_factory=list)
    forcing_gates: list[str] = Field(default_factory=list)
    decisive_probe_ids: list[str] = Field(default_factory=list)
    expected_learning: list[str] = Field(default_factory=list)
    decision_status: Literal["ready_to_run", "pursue", "pivot", "inconclusive"] = "ready_to_run"
    summary: str = ""
...
class DynamicPressureAutomatonWaveRequest(BaseModel):
    world_id: str | None = None
    max_window: int = Field(default=6, ge=1, le=8)
    modulus_extra_bits: int = Field(default=4, ge=0, le=6)
    max_probes: int = Field(default=8, ge=4, le=15)
    submit_after_compile: bool = False


class DynamicPressureAutomatonWaveRun(BaseModel):
    id: str = Field(default_factory=lambda: f"DP-{uuid4().hex[:10]}")
    campaign_id: str
    world_id: str | None = None
    world_label: str | None = None
    compiled_probe_count: int = 0
...
class PressureHeightFrontierCertificateWaveRequest(BaseModel):
    world_id: str | None = None
    max_window: int = Field(default=7, ge=2, le=8)
    modulus_extra_bits: int = Field(default=4, ge=0, le=6)
    max_probes: int = Field(default=12, ge=8, le=15)
...
class PressureHeightFrontierCertificateWaveRun(BaseModel):
    id: str = Field(default_factory=lambda: f"FC-{uuid4().hex[:10]}")
    campaign_id: str
    world_id: str | None = None
    world_label: str | None = None
    compiled_probe_count: int = 0
    ...
    probe_ids: list[str] = Field(default_factory=list)
    height_lift_reports: list[dict[str, Any]] = Field(default_factory=list)
    certificate_gates: list[str] = Field(default_factory=list)
    adversarial_gates: list[str] = Field(default_factory=list)
    decisive_probe_ids: list[str] = Field(default_factory=list)
    expected_learning: list[str] = Field(default_factory=list)
...
class PressureHeightFrontierCompletenessWaveRequest(BaseModel):
    world_id: str | None = None
    max_window: int = Field(default=8, ge=2, le=8)
    modulus_extra_bits: int = Field(default=4, ge=0, le=6)
    max_probes: int = Field(default=15, ge=10, le=15)
    submit_after_compile: bool = False


class PressureHeightFrontierCompletenessWaveRun(BaseModel):
    id: str = Field(default_factory=lambda: f"FK-{uuid4().hex[:10]}")
    campaign_id: str
    world_id: str | None = None
    world_label: str | None = None
    compiled_probe_count: int = 0
    ...
    probe_ids: list[str] = Field(default_factory=list)
    height_lift_reports: list[dict[str, Any]] = Field(default_factory=list)
    generated_frontier_reports: list[dict[str, Any]] = Field(default_factory=list)
    completeness_gates: list[str] = Field(default_factory=list)
    kill_gates: list[str] = Field(default_factory=list)
    decisive_probe_ids: list[str] = Field(default_factory=list)
    expected_learning: list[str] = Field(default_factory=list)
    finite_frontier_summary: str = ""
    decision_status: Literal["ready_to_run", "pursue", "pivot", "inconclusive"] = "ready_to_run"
    summary: str = ""
    created_at: datetime = Field(default_factory=_utc_now)


class PressureHeightParameterizedCompletenessWaveRequest(BaseModel):
    world_id: str | None = None
    max_probes: int = Field(default=13, ge=8, le=13)
    submit_after_compile: bool = False


class PressureHeightParameterizedCompletenessWaveRun(BaseModel):
    id: str = Field(default_factory=lambda: f"PK-{uuid4().hex[:10]}")
    campaign_id: str
    world_id: str | None = None
    world_label: str | None = None
    compiled_probe_count: int = 0
    submitted_probe_count: int = 0
    probe_ids: list[str] = Field(default_factory=list)
    parameterized_gates: list[str] = Field(default_factory=list)
    adversarial_gates: list[str] = Field(default_factory=list)
    decisive_probe_ids: list[str] = Field(default_factory=list)
    expected_learning: list[str] = Field(default_factory=list)
    theorem_target_summary: str = ""
    decision_status: Literal["ready_to_run", "pursue", "pivot", "inconclusive"] = "ready_to_run"
    summary: str = ""
    created_at: datetime = Field(default_factory=_utc_now)


class PressureHeightGeneratorBridgeWaveRequest(BaseModel):
    world_id: str | None = None
    max_probes: int = Field(default=13, ge=8, le=13)
    submit_after_compile: bool = False


class PressureHeightGeneratorBridgeWaveRun(BaseModel):
    id: str = Field(default_factory=lambda: f"GB-{uuid4().hex[:10]}")
    campaign_id: str
    world_id: str | None = None
    world_label: str | None = None
    compiled_probe_count: int = 0
    submitted_probe_count: int = 0
    probe_ids: list[str] = Field(default_factory=list)
    bridge_gates: list[str] = Field(default_factory=list)
...
class PressureHeightSccDriftWaveRequest(BaseModel):
    world_id: str | None = None
    max_probes: int = Field(default=13, ge=8, le=13)
    submit_after_compile: bool = False


class PressureHeightSccDriftWaveRun(BaseModel):
    id: str = Field(default_factory=lambda: f"SD-{uuid4().hex[:10]}")
    campaign_id: str
    world_id: str | None = None
    world_label: str | None = None
    compiled_probe_count: int = 0
    submitted_probe_count: int = 0
    probe_ids: list[str] = Field(default_factory=list)
    drift_gates: list[str] = Field(default_factory=list)
    obstruction_gates: list[str] = Field(default_factory=list)
    decisive_probe_ids: list[str] = Field(default_factory=list)
    expected_learning: list[str] = Field(default_factory=list)
    tranche_summary: str = ""
...
class PressureHeightFinalClosureWaveRequest(BaseModel):
    world_id: str | None = None
    max_probes: int = Field(default=13, ge=8, le=13)
    submit_after_compile: bool = False


class PressureHeightFinalClosureWaveRun(BaseModel):
    id: str = Field(default_factory=lambda: f"FC-{uuid4().hex[:10]}")
    campaign_id: str
    world_id: str | None = None
    world_label: str | None = None
    ...
    probe_ids: list[str] = Field(default_factory=list)
    closure_gates: list[str] = Field(default_factory=list)
    pullback_gates: list[str] = Field(default_factory=list)
    anti_circularity_gates: list[str] = Field(default_factory=list)
    decisive_probe_ids: list[str] = Field(default_factory=list)
    expected_learning: list[str] = Field(default_factory=list)
    phase_summary: str = ""
    decision_status: Literal["ready_to_run", "pursue", "pivot", "inconclusive"] = "ready_to_run"
