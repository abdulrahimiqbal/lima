    ...
    def run_proof(
        ...
        if lean_result["status"] == "formalization_failed":
            return ExecutionResult(
                ...
                artifacts=lean_result.get("artifacts", []),
                executor_backend="aristotle",
            )
            ...
            try:
                ...
                if project.status == ProjectStatus.FAILED:
                    return ExecutionResult(
                        ...
                        notes=f"Aristotle project {project.project_id} failed to find a proof. This does not mean the theorem is false.",
                        artifacts=[str(result_tar)] if result_tar.exists() else [],
                        executor_backend="aristotle",
                    )
                ...
                if "FAILED" in error.upper():
                    return ExecutionResult(
                        status="blocked",
                        failure_type="proof_failed",
                        notes=f"Aristotle could not find a proof (this does not mean the theorem is false): {error}",
                        executor_backend="aristotle",
                    )
                return ExecutionResult(
                    status="inconclusive",
