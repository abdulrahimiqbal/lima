...
class CampaignService:
    ...
    def run_pressure_height_parameterized_completeness_wave(
        ...
        with self._campaign_lock(campaign_id):
            ...
            world_id = payload.world_id or campaign.active_world_id or world_payload.get("id")
            if not world_id:
                raise KeyError("No promoted world is available for pressure-height parameterized completeness wave.")
            world_label = world_payload.get("label") or world_id
            probes = self._compile_pressure_height_parameterized_completeness_wave_probes(
                world_id=world_id,
                max_probes=payload.max_probes,
            )
            for probe in probes:
                self.memory.upsert_research_node(
                    ...
                    node_id=probe.id,
                    node_type="FormalProbe",
                    title=f"pressure-height-parameterized-completeness:{probe.probe_type}:{world_id}",
                    summary=probe.source_text,
                    status=probe.status,
                    payload=probe.model_dump(mode="json"),
                )
            run = PressureHeightParameterizedCompletenessWaveRun(
                campaign_id=campaign_id,
                world_id=world_id,
                world_label=world_label,
                compiled_probe_count=len(probes),
                probe_ids=[probe.id for probe in probes],
                parameterized_gates=[
                    "parameterized frontier generator invariant is explicit",
                    "generator invariant implies zero unchecked components at every depth",
                    "generator invariant implies recurrent bad components are covered by exits",
                    "generator invariant implies no dangerous frontier at every depth",
                    "bounded window-8 certificate is represented as an instance of the schema",
                ],
                adversarial_gates=[
                    "dangerous generator violates the invariant",
                    "unchecked generator violates the invariant",
                    "static persistent bad frontier is rejected without a dynamic witness",
                    "schema has no reachability or termination field",
                    "weak invariant exposes named obstruction instead of claiming closure",
                ],
                decisive_probe_ids=[
                    probe.id
                    for probe in probes
                ...
                expected_learning=[
                    "Whether the pressure-height route can be expressed as one parameterized completeness theorem.",
                    "Whether a generator invariant is strong enough to imply no dangerous frontier at all depths.",
                    "Whether adversarial dangerous/unchecked generators are rejected by the invariant.",
                    "Whether the remaining obstruction is a precise invariant-strengthening problem rather than another bounded search.",
                ],
                theorem_target_summary=(
                    "Parameterized target: every dynamically legal pressure-height frontier generated by "
                    "Collatz residue dynamics has all recurrent bad components certified by pressure recovery, "
                    "height escape, or survivor drop."
                ),
                summary=(
                    "Pressure-height parameterized completeness wave compiled the 13-probe theorem-lift attempt: "
                    "a generator invariant should imply completeness and no dangerous frontier at every depth, "
                    "while adversarial generators demonstrate the invariant is not vacuous."
                ),
            )
            self.memory.upsert_research_node(
                campaign_id=campaign_id,
                node_id=run.id,
                node_type="PressureHeightParameterizedCompletenessWaveRun",
                title=f"pressure-height-parameterized-completeness-wave:{world_id}",
                summary=run.summary,
                status=run.decision_status,
                payload=run.model_dump(mode="json"),
            )
...
""",
    ...
    def _compile_pressure_height_parameterized_completeness_wave_probes(
        self,
        *,
        world_id: str,
        max_probes: int,
        ...
        suffix = uuid4().hex[:8]
        base_defs = f"""
structure PHParamReport_{suffix} where
  depth : Nat
  recurrentBad : Nat
  pressureRecovery : Nat
  heightEscape : Nat
...
def phParamCertifiedCount_{suffix} (r : PHParamReport_{suffix}) : Nat :=
  r.pressureRecovery + r.heightEscape + r.survivorDrop

def phParamComplete_{suffix} (r : PHParamReport_{suffix}) : Prop :=
  r.unchecked = 0 /\\
    r.dangerous = 0 /\\
    r.recurrentBad <= phParamCertifiedCount_{suffix} r

def phParamNoDangerous_{suffix} (r : PHParamReport_{suffix}) : Prop :=
  r.dangerous = 0

def phParamNoUnchecked_{suffix} (r : PHParamReport_{suffix}) : Prop :=
  r.unchecked = 0

def phParamCoveredByExits_{suffix} (r : PHParamReport_{suffix}) : Prop :=
  r.recurrentBad <= phParamCertifiedCount_{suffix} r

structure PHParamGenerator_{suffix} where
  reportAt : Nat -> PHParamReport_{suffix}

def phParamGeneratorInvariant_{suffix} (gen : PHParamGenerator_{suffix}) : Prop :=
  ∀ depth, phParamComplete_{suffix} (gen.reportAt depth)

def phParamNoDangerousAtEveryDepth_{suffix} (gen : PHParamGenerator_{suffix}) : Prop :=
  ∀ depth, phParamNoDangerous_{suffix} (gen.reportAt depth)

def phParamNoUncheckedAtEveryDepth_{suffix} (gen : PHParamGenerator_{suffix}) : Prop :=
  ∀ depth, phParamNoUnchecked_{suffix} (gen.reportAt depth)

def phParamCoveredAtEveryDepth_{suffix} (gen : PHParamGenerator_{suffix}) : Prop :=
  ∀ depth, phParamCoveredByExits_{suffix} (gen.reportAt depth)

def phParamCompleteReport_{suffix} (depth : Nat) : PHParamReport_{suffix} :=
  {{ depth := depth, recurrentBad := 2, pressureRecovery := 0,
    heightEscape := 2, survivorDrop := 0, dangerous := 0, unchecked := 0 }}

def phParamAcyclicReport_{suffix} (depth : Nat) : PHParamReport_{suffix} :=
  {{ depth := depth, recurrentBad := 0, pressureRecovery := 0,
    heightEscape := 0, survivorDrop := 0, dangerous := 0, unchecked := 0 }}

def phParamWindow8Report_{suffix} : PHParamReport_{suffix} :=
  {{ depth := 8, recurrentBad := 2, pressureRecovery := 0,
    heightEscape := 2, survivorDrop := 0, dangerous := 0, unchecked := 0 }}

def phParamBoundedWindow8Generator_{suffix} : PHParamGenerator_{suffix} :=
  {{ reportAt := fun depth =>
      if depth = 0 then phParamAcyclicReport_{suffix} depth
      else phParamCompleteReport_{suffix} depth }}

def phParamDangerousReport_{suffix} (depth : Nat) : PHParamReport_{suffix} :=
  {{ depth := depth, recurrentBad := 1, pressureRecovery := 0,
    heightEscape := 0, survivorDrop := 0, dangerous := 1, unchecked := 0 }}

def phParamUncheckedReport_{suffix} (depth : Nat) : PHParamReport_{suffix} :=
  {{ depth := depth, recurrentBad := 1, pressureRecovery := 0,
    heightEscape := 0, survivorDrop := 0, dangerous := 0, unchecked := 1 }}

def phParamStaticPersistentBadReport_{suffix} (depth : Nat) : PHParamReport_{suffix} :=
  {{ depth := depth, recurrentBad := 1, pressureRecovery := 0,
    heightEscape := 0, survivorDrop := 0, dangerous := 1, unchecked := 0 }}

def phParamDangerousGenerator_{suffix} : PHParamGenerator_{suffix} :=
  {{ reportAt := phParamDangerousReport_{suffix} }}

def phParamUncheckedGenerator_{suffix} : PHParamGenerator_{suffix} :=
  {{ reportAt := phParamUncheckedReport_{suffix} }}

def phParamStaticPersistentBadGenerator_{suffix} : PHParamGenerator_{suffix} :=
  {{ reportAt := phParamStaticPersistentBadReport_{suffix} }}

structure PHParamCountingTarget_{suffix} where
  depth : Nat
  recurrentBad : Nat
  certified : Nat
  dangerous : Nat
...
def phParamCountingTargetAt_{suffix} (r : PHParamReport_{suffix}) : PHParamCountingTarget_{suffix} :=
  {{ depth := r.depth, recurrentBad := r.recurrentBad,
    certified := phParamCertifiedCount_{suffix} r,
    dangerous := r.dangerous, unchecked := r.unchecked }}

def phParamWeakInvariant_{suffix} (gen : PHParamGenerator_{suffix}) : Prop :=
  ∀ depth, (gen.reportAt depth).dangerous = 0

def phParamRemainingObstruction_{suffix} (gen : PHParamGenerator_{suffix}) : Prop :=
  phParamWeakInvariant_{suffix} gen /\\
    ∃ depth, (gen.reportAt depth).unchecked ≠ 0
""".strip()
        bounded_invariant = f"""
theorem ph_param_bounded_window8_generator_invariant_{suffix} :
    phParamGeneratorInvariant_{suffix} phParamBoundedWindow8Generator_{suffix} := by
  intro depth
  unfold phParamBoundedWindow8Generator_{suffix}
  by_cases h : depth = 0
  · simp [h, phParamComplete_{suffix}, phParamAcyclicReport_{suffix}, phParamCertifiedCount_{suffix}]
  · simp [h, phParamComplete_{suffix}, phParamCompleteReport_{suffix}, phParamCertifiedCount_{suffix}]
""".strip()
        specs: list[tuple[str, str, str, bool, str, str]] = [
            (
                "definition_probe",
                "Parameterized pressure-height completeness / generator invariant is definable.",
                f"""{base_defs}

{bounded_invariant}

theorem ph_param_generator_invariant_definable_{suffix} :
    phParamGeneratorInvariant_{suffix} phParamBoundedWindow8Generator_{suffix} := by
  exact ph_param_bounded_window8_generator_invariant_{suffix}
""",
                True,
            ...
            (
                "closure_probe",
                "Parameterized pressure-height completeness / invariant implies no unchecked component at every depth.",
                f"""{base_defs}

theorem ph_param_invariant_implies_no_unchecked_{suffix}
    (gen : PHParamGenerator_{suffix})
    (hInv : phParamGeneratorInvariant_{suffix} gen) :
    phParamNoUncheckedAtEveryDepth_{suffix} gen := by
  intro depth
  exact (hInv depth).1
""",
                True,
            ...
            (
                "closure_probe",
                "Parameterized pressure-height completeness / invariant covers recurrent bad components by exits.",
                f"""{base_defs}

theorem ph_param_invariant_covers_recurrent_bad_{suffix}
    (gen : PHParamGenerator_{suffix})
    (hInv : phParamGeneratorInvariant_{suffix} gen) :
    phParamCoveredAtEveryDepth_{suffix} gen := by
  intro depth
  exact (hInv depth).2.2
""",
                True,
                "parameterized_generator",
                "exit_coverage",
            ...
            (
                "closure_probe",
                "Parameterized pressure-height completeness / height-escape coverage is a complete report.",
                f"""{base_defs}

theorem ph_param_height_escape_coverage_complete_{suffix}
    (depth recurrent : Nat) :
    phParamComplete_{suffix}
      {{ depth := depth, recurrentBad := recurrent, pressureRecovery := 0,
        heightEscape := recurrent, survivorDrop := 0, dangerous := 0, unchecked := 0 }} := by
  unfold phParamComplete_{suffix} phParamCertifiedCount_{suffix}
  simp
""",
                True,
                "exit_preservation",
            ...
            (
                "closure_probe",
                "Parameterized pressure-height completeness / pressure-recovery coverage is a complete report.",
                f"""{base_defs}

theorem ph_param_pressure_recovery_coverage_complete_{suffix}
    (depth recurrent : Nat) :
    phParamComplete_{suffix}
      {{ depth := depth, recurrentBad := recurrent, pressureRecovery := recurrent,
        heightEscape := 0, survivorDrop := 0, dangerous := 0, unchecked := 0 }} := by
  unfold phParamComplete_{suffix} phParamCertifiedCount_{suffix}
  simp
""",
                True,
            ...
            (
                "closure_probe",
                "Parameterized pressure-height completeness / survivor-drop coverage is a complete report.",
                f"""{base_defs}

theorem ph_param_survivor_drop_coverage_complete_{suffix}
    (depth recurrent : Nat) :
    phParamComplete_{suffix}
      {{ depth := depth, recurrentBad := recurrent, pressureRecovery := 0,
        heightEscape := 0, survivorDrop := recurrent, dangerous := 0, unchecked := 0 }} := by
  unfold phParamComplete_{suffix} phParamCertifiedCount_{suffix}
  simp
""",
                True,
            ...
            (
                "closure_probe",
                "Parameterized pressure-height completeness / invariant implies no dangerous frontier at arbitrary depth.",
                f"""{base_defs}

theorem ph_param_invariant_implies_no_dangerous_{suffix}
    (gen : PHParamGenerator_{suffix})
    (hInv : phParamGeneratorInvariant_{suffix} gen) :
    phParamNoDangerousAtEveryDepth_{suffix} gen := by
  intro depth
  exact (hInv depth).2.1
""",
                True,
                "parameterized_generator",
                "no_dangerous",
            ...
            (
                "bridge_probe",
                "Parameterized pressure-height completeness / bounded window-8 certificate is an instance of the schema.",
                f"""{base_defs}

{bounded_invariant}

theorem ph_param_window8_is_schema_instance_{suffix} :
    phParamGeneratorInvariant_{suffix} phParamBoundedWindow8Generator_{suffix} /\\
    phParamComplete_{suffix} phParamWindow8Report_{suffix} := by
  constructor
  · exact ph_param_bounded_window8_generator_invariant_{suffix}
  · unfold phParamWindow8Report_{suffix} phParamComplete_{suffix} phParamCertifiedCount_{suffix}
    native_decide
""",
                True,
                "bounded_instance",
            ...
            (
                "anti_smuggling_probe",
                "Parameterized pressure-height completeness / schema has no reachability or termination field.",
                f"""{base_defs}

theorem ph_param_counting_target_has_no_reachability_field_{suffix} :
    (phParamCountingTargetAt_{suffix} phParamWindow8Report_{suffix}).recurrentBad = 2 /\\
    (phParamCountingTargetAt_{suffix} phParamWindow8Report_{suffix}).certified = 2 /\\
    (phParamCountingTargetAt_{suffix} phParamWindow8Report_{suffix}).dangerous = 0 /\\
    (phParamCountingTargetAt_{suffix} phParamWindow8Report_{suffix}).unchecked = 0 := by
  unfold phParamCountingTargetAt_{suffix} phParamWindow8Report_{suffix} phParamCertifiedCount_{suffix}
  native_decide
""",
                True,
                "anti_smuggling",
            ...
            (
                "anti_smuggling_probe",
                "Parameterized pressure-height completeness / dangerous generator violates the invariant.",
                f"""{base_defs}

theorem ph_param_dangerous_generator_violates_invariant_{suffix} :
    Not (phParamGeneratorInvariant_{suffix} phParamDangerousGenerator_{suffix}) := by
  intro hInv
  have hDangerZero := (hInv 0).2.1
  exact Nat.succ_ne_zero 0 hDangerZero
""",
                True,
            ...
            (
                "anti_smuggling_probe",
                "Parameterized pressure-height completeness / unchecked generator violates the invariant.",
                f"""{base_defs}

theorem ph_param_unchecked_generator_violates_invariant_{suffix} :
    Not (phParamGeneratorInvariant_{suffix} phParamUncheckedGenerator_{suffix}) := by
  intro hInv
  have hUncheckedZero := (hInv 0).1
  exact Nat.succ_ne_zero 0 hUncheckedZero
""",
                True,
                "adversarial_generator",
                "unchecked_rejected",
            ...
            (
                "anti_smuggling_probe",
                "Parameterized pressure-height completeness / static persistent bad frontier is rejected by dynamic invariant.",
                f"""{base_defs}

theorem ph_param_static_persistent_bad_rejected_{suffix} :
    Not (phParamGeneratorInvariant_{suffix} phParamStaticPersistentBadGenerator_{suffix}) := by
  intro hInv
  have hDangerZero := (hInv 0).2.1
  exact Nat.succ_ne_zero 0 hDangerZero
""",
            ...
            (
                "anti_smuggling_probe",
                "Parameterized pressure-height completeness / weak invariant exposes unchecked obstruction.",
                f"""{base_defs}

theorem ph_param_weak_invariant_exposes_named_obstruction_{suffix} :
    phParamRemainingObstruction_{suffix} phParamUncheckedGenerator_{suffix} := by
  constructor
  · intro depth
    rfl
  · exact Exists.intro 0 (Nat.succ_ne_zero 0)
""",
                True,
                "obstruction_schema",
                "weak_invariant_obstruction",
            ),
        ]
        probes: list[FormalProbe] = []
        for probe_type, source_text, lean, decisive, family, role in specs[:max_probes]:
            metadata = {
                "pressure_height_parameterized_completeness_wave": True,
                "parameterized_family": family,
                "parameterized_role": role,
                "decisive": decisive,
                "world_id": world_id,
                "max_probe_budget": 13,
            }
