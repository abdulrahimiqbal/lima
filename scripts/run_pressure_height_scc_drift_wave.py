...
DEFAULT_MEMORY_PATH = "/tmp/lima_pressure_height_scc_drift/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("SCC_DRIFT_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("SCC_DRIFT_MODE", "local")
    max_probes = int(os.environ.get("SCC_DRIFT_MAX_PROBES", "13"))

    service = build_service() if mode in {"submit", "poll"} else CampaignService(
        Settings(memory_db_path="/tmp/lima_pressure_height_scc_drift_local.db")
    )
    campaign_id = os.environ.get("SCC_DRIFT_CAMPAIGN_ID")
    if mode == "local":
        campaign = create_campaign(service)
        run = service.run_pressure_height_scc_drift_wave(
            campaign.id,
            PressureHeightSccDriftWaveRequest(
                world_id=WORLD_ID,
    ...
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("SCC_DRIFT_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(f"Unsupported SCC_DRIFT_MODE={mode!r}; use local, submit, or poll")
...
def create_campaign(service: CampaignService):
    return service.create_campaign(
        CampaignCreate(
            title="Local pressure-height SCC drift Aristotle run",
            problem_statement="Collatz pressure-height SCC drift tranche probe run.",
            operator_notes=[
                "SCC drift tranche after exactness tranche passed.",
                "Submit this before route-integration tranche.",
