...
DEFAULT_MEMORY_PATH = "/tmp/lima_dynamic_admissibility_compass/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("DYNAMIC_COMPASS_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("DYNAMIC_COMPASS_MODE", "submit")
    service = build_service()
    campaign_id = os.environ.get("DYNAMIC_COMPASS_CAMPAIGN_ID")
    ...
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("DYNAMIC_COMPASS_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(f"Unsupported DYNAMIC_COMPASS_MODE={mode!r}; use submit or poll")
...
def submit(service: CampaignService, campaign_id: str | None) -> None:
    ...
    run = service.run_dynamic_admissibility_compass_wave(
        campaign_id,
        DynamicAdmissibilityCompassWaveRequest(
            world_id=WORLD_ID,
            max_probes=15,
    ...
    emit(
        "compiled",
        {
            ...
            "compass_gates": run.compass_gates,
            "pivot_sentinels": run.pivot_sentinels,
        },
    )
...
def create_campaign(service: CampaignService):
    return service.create_campaign(
        CampaignCreate(
            title="Local dynamic admissibility compass Aristotle run",
            problem_statement="Collatz dynamic-admissibility bridge plus pivot-sentinel probe run.",
            operator_notes=[
