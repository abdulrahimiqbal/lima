...
from app.collatz_automaton import analyze_height_lifted_pressure_automaton
...
DEFAULT_MEMORY_PATH = "/tmp/lima_pressure_height_frontier_completeness/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("FRONTIER_COMPLETENESS_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("FRONTIER_COMPLETENESS_MODE", "local")
    max_window = int(os.environ.get("FRONTIER_COMPLETENESS_MAX_WINDOW", "8"))
    extra_bits = int(os.environ.get("FRONTIER_COMPLETENESS_EXTRA_BITS", "4"))
    max_probes = int(os.environ.get("FRONTIER_COMPLETENESS_MAX_PROBES", "15"))
    ...
    campaign_id = os.environ.get("FRONTIER_COMPLETENESS_CAMPAIGN_ID")
    if mode == "submit":
        submit(service, campaign_id, max_window=max_window, extra_bits=extra_bits, max_probes=max_probes)
        return
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("FRONTIER_COMPLETENESS_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(
        f"Unsupported FRONTIER_COMPLETENESS_MODE={mode!r}; use local, submit, or poll"
    )
...
def submit(
    ...
    *,
    ...
    run = service.run_pressure_height_frontier_completeness_wave(
        campaign_id,
        PressureHeightFrontierCompletenessWaveRequest(
            world_id=WORLD_ID,
    ...
    emit(
        "compiled",
        {
            ...
            "finite_frontier_summary": run.finite_frontier_summary,
            "completeness_gates": run.completeness_gates,
            "kill_gates": run.kill_gates,
            "generated_frontier_reports": run.generated_frontier_reports,
        },
    )
...
def create_campaign(service: CampaignService):
    return service.create_campaign(
        CampaignCreate(
            title="Local pressure-height frontier completeness Aristotle run",
            problem_statement="Collatz pressure-plus-height frontier completeness kill-test run.",
            operator_notes=[
                "Completeness kill-test after R20 frontier certificate theorem passed.",
