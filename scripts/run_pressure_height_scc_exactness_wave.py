...
DEFAULT_MEMORY_PATH = "/tmp/lima_pressure_height_scc_exactness/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("SCC_EXACTNESS_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("SCC_EXACTNESS_MODE", "local")
    max_probes = int(os.environ.get("SCC_EXACTNESS_MAX_PROBES", "13"))

    service = build_service() if mode in {"submit", "poll"} else CampaignService(
        Settings(memory_db_path="/tmp/lima_pressure_height_scc_exactness_local.db")
    )
    campaign_id = os.environ.get("SCC_EXACTNESS_CAMPAIGN_ID")
    if mode == "local":
        campaign = create_campaign(service)
        run = service.run_pressure_height_scc_exactness_wave(
            campaign.id,
            PressureHeightSccExactnessWaveRequest(
                world_id=WORLD_ID,
    ...
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("SCC_EXACTNESS_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(f"Unsupported SCC_EXACTNESS_MODE={mode!r}; use local, submit, or poll")
    ...
    *,
    ...
    emit(
        "compiled",
        {
            ...
            "exactness_gates": run.exactness_gates,
            "obstruction_gates": run.obstruction_gates,
        },
    )
...
def create_campaign(service: CampaignService):
    return service.create_campaign(
        CampaignCreate(
            title="Local pressure-height SCC exactness Aristotle run",
            problem_statement="Collatz pressure-height SCC exactness tranche probe run.",
            operator_notes=[
                "SCC exactness tranche after R23 actual-generator bridge passed.",
                "Submit this before spending a separate drift tranche.",
