...
DEFAULT_MEMORY_PATH = "/tmp/lima_pressure_height_survivor_closure/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("SURVIVOR_CLOSURE_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("SURVIVOR_CLOSURE_MODE", "local")
    max_window = int(os.environ.get("SURVIVOR_CLOSURE_MAX_WINDOW", "7"))
    extra_bits = int(os.environ.get("SURVIVOR_CLOSURE_EXTRA_BITS", "4"))
    max_probes = int(os.environ.get("SURVIVOR_CLOSURE_MAX_PROBES", "10"))
    ...
    campaign_id = os.environ.get("SURVIVOR_CLOSURE_CAMPAIGN_ID")
    if mode == "submit":
        submit(service, campaign_id, max_window=max_window, extra_bits=extra_bits, max_probes=max_probes)
        return
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("SURVIVOR_CLOSURE_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(f"Unsupported SURVIVOR_CLOSURE_MODE={mode!r}; use local, submit, or poll")
...
def submit(
    ...
    *,
    ...
    run = service.run_pressure_height_survivor_closure_wave(
        campaign_id,
        PressureHeightSurvivorClosureWaveRequest(
            world_id=WORLD_ID,
    ...
    emit(
        "compiled",
        {
            ...
            "closure_summary": run.closure_summary,
            "closure_gates": run.closure_gates,
...
def create_campaign(service: CampaignService):
    return service.create_campaign(
        CampaignCreate(
            title="Local pressure-height survivor closure Aristotle run",
            problem_statement="Collatz pressure-plus-height survivor-closure probe run.",
            operator_notes=[
                "Search-backed survivor closure run after height-lifted pressure automaton.",
