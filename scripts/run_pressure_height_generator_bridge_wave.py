...
DEFAULT_MEMORY_PATH = "/tmp/lima_pressure_height_generator_bridge/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("GENERATOR_BRIDGE_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("GENERATOR_BRIDGE_MODE", "local")
    max_probes = int(os.environ.get("GENERATOR_BRIDGE_MAX_PROBES", "13"))

    service = build_service() if mode in {"submit", "poll"} else CampaignService(
        Settings(memory_db_path="/tmp/lima_pressure_height_generator_bridge_local.db")
    )
    campaign_id = os.environ.get("GENERATOR_BRIDGE_CAMPAIGN_ID")
    if mode == "local":
        campaign = create_campaign(service)
        run = service.run_pressure_height_generator_bridge_wave(
            campaign.id,
            PressureHeightGeneratorBridgeWaveRequest(
                world_id=WORLD_ID,
    ...
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("GENERATOR_BRIDGE_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(f"Unsupported GENERATOR_BRIDGE_MODE={mode!r}; use local, submit, or poll")
    ...
    *,
    ...
    emit(
        "compiled",
        {
            ...
            "bridge_summary": run.bridge_summary,
            "bridge_gates": run.bridge_gates,
...
def create_campaign(service: CampaignService):
    return service.create_campaign(
        CampaignCreate(
            title="Local pressure-height generator bridge Aristotle run",
            problem_statement="Collatz pressure-height actual-generator bridge probe run.",
            operator_notes=[
                "Actual-generator bridge wave after R22 parameterized schema passed.",
