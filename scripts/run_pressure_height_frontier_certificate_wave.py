...
DEFAULT_MEMORY_PATH = "/tmp/lima_pressure_height_frontier_certificate/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("FRONTIER_CERT_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("FRONTIER_CERT_MODE", "local")
    max_window = int(os.environ.get("FRONTIER_CERT_MAX_WINDOW", "7"))
    extra_bits = int(os.environ.get("FRONTIER_CERT_EXTRA_BITS", "4"))
    max_probes = int(os.environ.get("FRONTIER_CERT_MAX_PROBES", "12"))
    ...
    campaign_id = os.environ.get("FRONTIER_CERT_CAMPAIGN_ID")
    if mode == "submit":
        submit(service, campaign_id, max_window=max_window, extra_bits=extra_bits, max_probes=max_probes)
        return
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("FRONTIER_CERT_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(f"Unsupported FRONTIER_CERT_MODE={mode!r}; use local, submit, or poll")
...
def submit(
    ...
    *,
    ...
    run = service.run_pressure_height_frontier_certificate_wave(
        campaign_id,
        PressureHeightFrontierCertificateWaveRequest(
            world_id=WORLD_ID,
    ...
    emit(
        "compiled",
        {
            ...
            "certificate_summary": run.certificate_summary,
            "certificate_gates": run.certificate_gates,
...
def create_campaign(service: CampaignService):
    return service.create_campaign(
        CampaignCreate(
            title="Local pressure-height frontier certificate Aristotle run",
            problem_statement="Collatz pressure-plus-height frontier certificate probe run.",
            operator_notes=[
                "Certificate theorem run after pressure-height survivor closure passed.",
