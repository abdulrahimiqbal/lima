...
def main() -> None:
    ...
    memory_path = Path("/tmp/lima_pivot_portfolio/lima_memory.db")
    ...
    campaign = service.create_campaign(
        CampaignCreate(
            title="Local pivot portfolio Aristotle run",
            problem_statement="Collatz conjecture pivot portfolio probe run.",
            operator_notes=[
    ...
    run = service.run_pivot_portfolio_wave(
        campaign.id,
        PivotPortfolioWaveRequest(
            world_id=WORLD_ID,
            max_probes=32,
    ...
    emit(
        "compiled",
        {
            ...
            "pivot_families": run.pivot_families,
        },
    )
    ...
    emit(
        "digest",
        {
            ...
            "diagnostics": [
                ...
                and str(item.get("source_text", "")).startswith("Pivot portfolio")
            ],
        },
    )
