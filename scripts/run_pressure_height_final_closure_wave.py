...
DEFAULT_MEMORY_PATH = "/tmp/lima_pressure_height_final_closure/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("FINAL_CLOSURE_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("FINAL_CLOSURE_MODE", "local")
    max_probes = int(os.environ.get("FINAL_CLOSURE_MAX_PROBES", "13"))

    service = build_service() if mode in {"submit", "poll"} else CampaignService(
        Settings(memory_db_path="/tmp/lima_pressure_height_final_closure_local.db")
    )
    campaign_id = os.environ.get("FINAL_CLOSURE_CAMPAIGN_ID")
    if mode == "local":
        campaign = create_campaign(service)
        run = service.run_pressure_height_final_closure_wave(
            campaign.id,
            PressureHeightFinalClosureWaveRequest(
                world_id=WORLD_ID,
    ...
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("FINAL_CLOSURE_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(f"Unsupported FINAL_CLOSURE_MODE={mode!r}; use local, submit, or poll")
    ...
    *,
    ...
    emit(
        "compiled",
        {
            ...
            "pullback_gates": run.pullback_gates,
            "anti_circularity_gates": run.anti_circularity_gates,
        },
    )
...
def create_campaign(service: CampaignService):
    return service.create_campaign(
        CampaignCreate(
            title="Local pressure-height final closure Aristotle run",
            problem_statement="Collatz pressure-height final global closure and pullback probe run.",
            operator_notes=[
                "Final global closure / ordinary pullback phase after route integration passed.",
                "This is still verification-as-discovery, not a claim of solved Collatz before digest.",
