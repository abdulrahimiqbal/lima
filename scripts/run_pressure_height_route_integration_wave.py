...
DEFAULT_MEMORY_PATH = "/tmp/lima_pressure_height_route_integration/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("ROUTE_INTEGRATION_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("ROUTE_INTEGRATION_MODE", "local")
    max_probes = int(os.environ.get("ROUTE_INTEGRATION_MAX_PROBES", "13"))

    service = build_service() if mode in {"submit", "poll"} else CampaignService(
        Settings(memory_db_path="/tmp/lima_pressure_height_route_integration_local.db")
    )
    campaign_id = os.environ.get("ROUTE_INTEGRATION_CAMPAIGN_ID")
    if mode == "local":
        campaign = create_campaign(service)
        run = service.run_pressure_height_route_integration_wave(
            campaign.id,
            PressureHeightRouteIntegrationWaveRequest(
                world_id=WORLD_ID,
    ...
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("ROUTE_INTEGRATION_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(f"Unsupported ROUTE_INTEGRATION_MODE={mode!r}; use local, submit, or poll")
    ...
    *,
    ...
    emit(
        "compiled",
        {
            ...
            "tranche_summary": run.tranche_summary,
            "integration_gates": run.integration_gates,
            "remaining_debt_gates": run.remaining_debt_gates,
        },
    )
...
def create_campaign(service: CampaignService):
    return service.create_campaign(
        CampaignCreate(
            title="Local pressure-height route integration Aristotle run",
            problem_statement="Collatz pressure-height route integration tranche probe run.",
            operator_notes=[
                "Route integration tranche after exactness and drift tranches passed.",
                "This tests proof-spine composition, not final Collatz pullback.",
