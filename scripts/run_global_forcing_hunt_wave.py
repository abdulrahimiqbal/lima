...
DEFAULT_MEMORY_PATH = "/tmp/lima_global_forcing_hunt/lima_memory.db"
POLL_SECONDS = 20
...
def build_service() -> CampaignService:
    ...
    memory_path = Path(os.environ.get("GLOBAL_FORCING_MEMORY_PATH", DEFAULT_MEMORY_PATH))
...
def main() -> None:
    mode = os.environ.get("GLOBAL_FORCING_MODE", "submit")
    service = build_service()
    campaign_id = os.environ.get("GLOBAL_FORCING_CAMPAIGN_ID")
    ...
    if mode == "poll":
        if not campaign_id:
            raise SystemExit("GLOBAL_FORCING_CAMPAIGN_ID is required for poll mode")
        poll_to_empty(service, campaign_id)
        digest(service, campaign_id)
        return
    raise SystemExit(f"Unsupported GLOBAL_FORCING_MODE={mode!r}; use submit or poll")
...
def submit(service: CampaignService, campaign_id: str | None) -> None:
    if campaign_id:
        try:
            ...
            campaign = service.create_campaign(
                CampaignCreate(
                    title="Local global forcing hunt Aristotle run",
                    problem_statement="Collatz global Composite Scarcity forcing adversarial probe run.",
                    operator_notes=[
    ...
    run = service.run_global_forcing_hunt_wave(
        campaign_id,
        GlobalForcingHuntWaveRequest(
            world_id=WORLD_ID,
            max_probes=12,
    ...
    emit(
        "compiled",
        {
            ...
            "forcing_gates": run.forcing_gates,
        },
    )
    ...
    emit(
        "digest",
        {
            ...
            "diagnostics": [
                ...
                and str(item.get("source_text", "")).startswith("Global forcing hunt")
            ],
        },
    )
